class Solution:
    ...
    def get_kth_smallest(self, nums1, start1, end1, nums2, start2, end2, k):
        ...
        # 而k变成k-(j-start2+1)，即减去逻辑上排出的元素的个数（要加1，索引相减）
        if nums1[i] > nums2[j]:
...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        len2 = len(nums2)
        l = (len1 + len2 + 1) // 2;
        r = (len1 + len2 + 2) // 2  # 奇数偶数情况
        return 0.5 * (self.kthsmall(nums1, nums2, 0, len1 - 1, 0, len2 - 1, l) \
                      + self.kthsmall(nums1, nums2, 0, len1 - 1, 0, len2 - 1, r))

    def kthsmall(self, nums1, nums2, start1, end1, start2, end2, k):
        len1 = end1 - start1 + 1  # length of first array
        len2 = end2 - start2 + 1  # length of second array
        if k == 1: return min(nums1[start1], nums2[start2])
        if len1 > len2: return self.kthsmall(nums2, nums1, start2, end2, start1, end1, k)
        if len1 == 0: return nums2[start2 + k - 1]
        i = start1 + min(len1, k // 2) - 1
        j = start2 + min(len2, k // 2) - 1
        if nums1[i] > nums2[j]:
            return self.kthsmall(nums1, nums2, start1, end1, j + 1, end2, k - (j - start2 + 1))
        else:
            return self.kthsmall(nums1, nums2, i + 1, end1, start2, end2, k - (i - start1 + 1))